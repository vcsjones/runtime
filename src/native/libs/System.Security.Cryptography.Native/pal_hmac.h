...
// A struct that contains the HMAC context, depending on which HMAC mechanism is used.
// Some OpenSSL 3.0 versions require remembering the key because they context cannot re-initialize itself with an existing
// key.
struct _DN_MAC_CTX {
    HMAC_CTX* legacy;
    EVP_MAC_CTX* mac;

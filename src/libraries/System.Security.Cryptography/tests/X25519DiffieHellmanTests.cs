...
namespace System.Security.Cryptography.Tests
{
    ...
    public static class X25519DiffieHellmanTests
    {
        ...
        public static void ImportSubjectPublicKeyInfo_TrailingData()
        {
            ...
            Assert.Throws<CryptographicException>(() => X25519DiffieHellman.ImportSubjectPublicKeyInfo(oversized));
            Assert.Throws<CryptographicException>(() => X25519DiffieHellman.ImportSubjectPublicKeyInfo(new ReadOnlySpan<byte>(oversized)));
        }
        ...
        public static void ImportPkcs8PrivateKey_WrongAlgorithm()
        {
            ...
            Assert.Throws<CryptographicException>(() => X25519DiffieHellman.ImportPkcs8PrivateKey(new ReadOnlySpan<byte>(ecP256Key)));
            Assert.Throws<CryptographicException>(() => X25519DiffieHellman.ImportPkcs8PrivateKey(ecP256Key));
        }
        ...
        public static void ImportPkcs8PrivateKey_BogusAsnChoice(bool useSpanImport)
        {
            ...
            if (useSpanImport)
            {
                Assert.Throws<CryptographicException>(() => X25519DiffieHellman.ImportPkcs8PrivateKey(new ReadOnlySpan<byte>(pkcs8)));
            }
            else
            {
        ...
        {
            // RFC 8410: AlgorithmIdentifier parameters MUST be absent
            byte[] pkcs8 = Pkcs8Encode(X25519DiffieHellmanTestData.X25519Oid, X25519DiffieHellmanTestData.AlicePrivateKey, algorithmParameters: s_asnNull);

...
internal static partial class Interop
{
    internal static partial class AppleCrypto
    {
        ...
        [UnmanagedCallConv(CallConvs = [ typeof(CallConvSwift) ])]
        private static unsafe partial SafeAppleCryptoSymmetricKeyHandle AppleCryptoNative_SymmetricKeyImport(
            UnsafeBufferPointer<byte> source);

        internal static unsafe SafeAppleCryptoSymmetricKeyHandle SymmetricKeyImport(ReadOnlySpan<byte> source)
        {
            SafeAppleCryptoSymmetricKeyHandle key;

            fixed (byte* sourcePtr = source)
            {
                key = AppleCryptoNative_SymmetricKeyImport(
                    new UnsafeBufferPointer<byte>(sourcePtr, source.Length));
            }
